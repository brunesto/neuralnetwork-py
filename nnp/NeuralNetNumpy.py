...
class NeuralNetNumpy:
    def __init__(self, config):
        ...
        for li in range(1, len(config.layer_sizes)):
            self.ws.append(np.zeros((config.layer_sizes[li], config.layer_sizes[li - 1])))

        # biases
        self.bs = []
        for li in range(1, len(config.layer_sizes)):
            self.bs.append(np.zeros((config.layer_sizes[li])))
    ...
    def reset_dls(self):
        ...
        for i in range(1, len(self.config.layer_sizes)):
            self.dls.append(np.zeros((self.config.layer_sizes[i])))
    ...
    def reset_dws(self):
        ...
        for li in range(1, len(self.config.layer_sizes)):
            self.dws.append(np.zeros((self.config.layer_sizes[li], self.config.layer_sizes[li - 1] )))

        self.dbs = []
        for li in range(1, len(self.config.layer_sizes)):
            self.dbs.append(np.zeros((self.config.layer_sizes[li])))
